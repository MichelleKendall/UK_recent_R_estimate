...
estimate_R_func <- function(incid,
  ...
  if (si_uncertainty == "Y") {
    ...
    quantile_0.10_posterior <- qgamma(0.1,
                                      shape = a_posterior,
                                      scale = b_posterior, lower.tail = TRUE, log.p = FALSE
    )
    ...
    quantile_0.90_posterior <- qgamma(0.9,
                                      shape = a_posterior,
                                      scale = b_posterior, lower.tail = TRUE, log.p = FALSE
    )
  ...
  results <- list(R = as.data.frame(cbind(
    ...
    quantile_0.10_posterior,
    quantile_0.25_posterior, median_posterior, quantile_0.75_posterior,
    quantile_0.90_posterior,
    quantile_0.95_posterior, quantile_0.975_posterior
  )))
  
  names(results$R) <- c(
    "t_start", "t_end", "Mean(R)", "Std(R)",
    "Quantile.0.025(R)", "Quantile.0.05(R)", 
    "Quantile.0.10(R)", "Quantile.0.25(R)",
    "Median(R)", "Quantile.0.75(R)",
    "Quantile.0.90(R)", "Quantile.0.95(R)",
    "Quantile.0.975(R)"
  )
